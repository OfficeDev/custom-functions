...
(function (OSFPerfUtil) {
    ...
    function sendPerformanceTelemetry() {
        if (typeof OTel !== "undefined" && OSF.AppTelemetry.enableTelemetry && typeof OSFPerformance !== "undefined") {
            var hostPerfResource;
            var officePerfResource;
            var resources = performance.getEntriesByType("resource");
            resources.forEach(function (resource) {
                if (resource.name.endsWith(OSFPerformance.hostSpecificFileName)) {
                    hostPerfResource = resource;
                }
                else if (resource.name.endsWith(OSF.ConstantNames.OfficeDebugJS) || resource.name.endsWith(OSF.ConstantNames.OfficeJS)) {
                    officePerfResource = resource;
                }
            });
            OTel.OTelLogger.onTelemetryLoaded(function () {
                var dataFields = prepareDataFieldsForOtel(hostPerfResource, "HostJs");
                dataFields = dataFields.concat(prepareDataFieldsForOtel(officePerfResource, "OfficeJs"));
...
(function (OfficeExt) {
    var LifeCycle;
    (function (LifeCycle) {
        var ExtensionIdentity = (function () {
            function ExtensionIdentity(id, storeId, storeType, appVersion) {
                this.Id = id;
        ...
        LifeCycle.ExtensionIdentity = ExtensionIdentity;
        var HostProperties = (function () {
            function HostProperties(hostType, formFactor, locale) {
        ...
        LifeCycle.HostProperties = HostProperties;
        var ExtensionLaunchProperties = (function () {
            function ExtensionLaunchProperties(optionalProperties, devProperties) {
                this.OptionalProperties = optionalProperties;
                this.DevProperties = devProperties;
            }
            return ExtensionLaunchProperties;
        })();
        LifeCycle.ExtensionLaunchProperties = ExtensionLaunchProperties;
        var LaunchParameters = (function () {
            function LaunchParameters(extId, componentType, componentId, hostProps, optionalProps) {
                this.ExtId = extId;
        ...
        LifeCycle.LaunchParameters = LaunchParameters;
        var ExtensionComponentInstance = (function () {
            function ExtensionComponentInstance(extId, componentType, componentId, instanceToken, launchProps) {
                this.Id = extId;
                this.ComponentType = componentType;
                this.ComponentId = componentId;
                this.InstanceToken = instanceToken;
                this.LaunchProperties = launchProps;
            }
            ExtensionComponentInstance.prototype.SetLaunchProperties = function (launchProps) {
                this.LaunchProperties = launchProps;
            };
            ExtensionComponentInstance.prototype.stop = function () {
                if (OSF.ExtensionLifeCycle && OSF.ExtensionLifeCycle.stopExtensionComponent) {
                    OSF.ExtensionLifeCycle.stopExtensionComponent(this);
                }
            };
            ExtensionComponentInstance.prototype.restart = function () {
                if (OSF.ExtensionLifeCycle && OSF.ExtensionLifeCycle.restartExtensionComponent) {
                    OSF.ExtensionLifeCycle.restartExtensionComponent(this);
                }
            };
            return ExtensionComponentInstance;
        })();
        LifeCycle.ExtensionComponentInstance = ExtensionComponentInstance;
        var ExtensionLifeCycleManager = (function () {
            function ExtensionLifeCycleManager() {
            }
            ExtensionLifeCycleManager.prototype.launchExtensionComponent = function (extId, componentType, componentId, hostProps, optionalProps) {
                if (OSF.ExtensionLifeCycle && OSF.ExtensionLifeCycle.launchExtensionComponent) {
                    var instanceToken = OSF.OUtil.Guid.generateNewGuid();
                    if (optionalProps == null) {
                        optionalProps = new ExtensionLaunchProperties({}, {});
                    }
                    optionalProps.OptionalProperties.InstanceToken = instanceToken;
                    var params = new LaunchParameters(extId, componentType, componentId, hostProps, optionalProps);
                    OSF.ExtensionLifeCycle.launchExtensionComponent(params);
                    var instance = new ExtensionComponentInstance(extId, componentType, componentId, instanceToken, optionalProps);
        ...
        LifeCycle.ExtensionLifeCycleManager = ExtensionLifeCycleManager;
    })(LifeCycle = OfficeExt.LifeCycle || (OfficeExt.LifeCycle = {}));
})(OfficeExt || (OfficeExt = {}));
...
(function (OfficeExt) {
    var UI;
    (function (UI) {
        var TaskPaneHeaderButtonParameters = (function () {
            function TaskPaneHeaderButtonParameters(label, icon) {
                this.Label = label;
                this.Icon = icon;
            }
            return TaskPaneHeaderButtonParameters;
        })();
        UI.TaskPaneHeaderButtonParameters = TaskPaneHeaderButtonParameters;
        var TaskPaneAction = (function () {
            function TaskPaneAction() {
            }
            TaskPaneAction.prototype.enableHeaderButton = function (label, icon, onClick) {
                if (OSF.TaskPaneAction && OSF.TaskPaneAction.enableHeaderButton) {
                    var params = new TaskPaneHeaderButtonParameters(label, icon);
                    OSF.TaskPaneAction.enableHeaderButton(params);
                    this.onHeaderButtonClick = onClick;
                }
            };
            TaskPaneAction.prototype.disableHeaderButton = function () {
                if (OSF.TaskPaneAction && OSF.TaskPaneAction.disableHeaderButton) {
                    OSF.TaskPaneAction.disableHeaderButton();
                }
            };
            return TaskPaneAction;
        })();
        UI.TaskPaneAction = TaskPaneAction;
    })(UI = OfficeExt.UI || (OfficeExt.UI = {}));
})(OfficeExt || (OfficeExt = {}));
...
addEventListener("JSPerfFinished", function () {
...
OSF.InitializationHelper.prototype.prepareRightBeforeWebExtensionInitialize = function OSF_InitializationHelper$prepareRightBeforeWebExtensionInitialize(appContext) {
    ...
    appContext.extensionLifeCycle = new OfficeExt.LifeCycle.ExtensionLifeCycleManager();
    appContext.ui.taskPaneAction = new OfficeExt.UI.TaskPaneAction();
